...
def lite():
    """简易定价接口"""
    option = DoubleBarrierOption(maturity=1, strike=100, bound=(80, 120), parti=0.5,
                                 inout=InOut.In, callput=CallPut.Call,
                                 s=100, r=0.03, q=0.03, vol=0.2)
...
def run():
    ...
    mc_engine = MCDoubleBarrierEngine(process, n_path=100000, rands_method=RandsMethod.Pseudorandom,
    ...
    for inout in InOut:
        for callput in CallPut:
            # 离散观察(每日观察)
            option_discrete = DoubleBarrierOption(maturity=1, start_date=datetime.date(2021, 1, 5),
                                                  strike=100, inout=inout, callput=callput, parti=0.5,
                                                  bound=(80, 120), rebate=(0, 0), window=(None, None),
                                                  discrete_obs_interval=1 / t_step_per_year,
                                                  exercise_type=ExerciseType.American,
                                                  payment_type=PaymentType.Expire, t_step_per_year=t_step_per_year)
            # 连续观察
            option_continuous = DoubleBarrierOption(maturity=1, start_date=datetime.date(2021, 1, 5),
                                                    strike=100, inout=inout, callput=callput, parti=0.5,
                                                    bound=(80, 120), rebate=(0, 0), window=(None, None),
    ...
    df2 = pd.DataFrame(results_continuous, columns=['障碍期权(连续观察)', 'I&K1992', 'Haug1998', 'PDE'])
    return df1, df2
