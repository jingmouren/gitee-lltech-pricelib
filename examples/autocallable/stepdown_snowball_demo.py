...
def lite():
    """简易定价接口"""
    option = StepDownSnowball(maturity=2, lock_term=3, s0=100, barrier_out_start=103, barrier_out_step=0.5,
                              barrier_in=75, coupon_out=0.126, s=100, r=0.02, q=0.04, vol=0.16)
...
def run():
    ...
    # 4. 定义产品：降敲雪球
    option = StepDownSnowball(maturity=2, s0=100, start_date=datetime.date(2022, 1, 5),
                              trade_calendar=CN_CALENDAR, barrier_out_start=103, barrier_out_step=0.5, barrier_in=75,
                              coupon_out=0.126, coupon_div=None,
                              lock_term=3, pay_dates=None, engine=None, status=StatusType.NoTouch,
                              t_step_per_year=243)
    # 5.为产品设置定价引擎
