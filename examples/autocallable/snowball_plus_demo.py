...
def lite():
    """简易定价接口"""
    option = SnowballPlus(maturity=2, lock_term=3, s0=100, barrier_out=103, barrier_in=80, coupon_out=0.126,
                          parti_out=0.5, strike_call=105, s=100, r=0.02, q=0.03, vol=0.15)
...
def run():
    ...
    option = SnowballPlus(maturity=2, s0=100, start_date=datetime.date(2022, 1, 5),
                          trade_calendar=CN_CALENDAR, barrier_out=103, barrier_in=80, coupon_out=0.126, coupon_div=None,
                          lock_term=3, parti_out=0.5, strike_call=105, engine=None, status=StatusType.NoTouch,
                          t_step_per_year=243)
    # 5.为产品设置定价引擎
