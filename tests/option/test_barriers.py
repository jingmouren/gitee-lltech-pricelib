...
def test_barrier_demo():
    expected_discrete = [[110, '离散观察向上敲出看涨期权', 0.14541706951380462, 0.1449829265364823, 0.1442251480892973,
                          0.14448855582014658, 0.14569170345643637],
                         [110, '离散观察向上敲出看跌期权', 7.003389638583823, 7.013881690900373, 6.992907472607452,
                          6.970243047865854, 7.0157816550155605],
                         [90, '离散观察向下敲出看涨期权', 5.596471515410604, 5.609916847782886, 5.590733350151679,
                          5.563814103031952, 5.599437648181393],
                         [90, '离散观察向下敲出看跌期权', 0.2157089603993949, 0.20303968340901607, 0.2140492615642641,
                          0.1921376752340182, 0.21476058332335518],
                         [110, '离散观察向上敲入看涨期权', 6.585500579649498, 6.590661212031636, 6.575884349538407,
                          6.584940982094393, 6.580718980409329],
                         [110, '离散观察向上敲入看跌期权', 1.6491389153588862, 1.6440599129752898, 1.6412198429599278,
                          1.6810912768961297, 1.6319304101407335],
                         [90, '离散观察向下敲入看涨期权', 1.1344461337526983, 1.1257272907852338, 1.1293881342669085,
                          1.1655900255215395, 1.126973035684375],
                         [90, '离散观察向下敲入看跌期权', 8.436819593543314, 8.454901920466648, 8.420090040802872,
...
def test_double_shark_demo():
    ...
    expected_discrete_df = pd.DataFrame(expected_discrete,
                                        columns=['双鲨期权(每日观察)', 'I&K1992', 'Haug1998', 'MonteCarlo', 'PDE'])
    expected_continuous_df = pd.DataFrame(expected_continuous,
